    ...
    <header class="sticky top-0 z-50 w-full bg-slate-50 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-200 dark:border-white/5 transition-all duration-300">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center">
                <img src="{{ asset('images/logo.jpeg') }}" alt="Chandla Book" class="h-14 w-auto mr-3" decoding="async">
            ...
            <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                <a href="{{ route('public.contact') }}" class="px-4 py-2 text-slate-900 dark:text-white/80 hover:text-slate-900 dark:hover:text-white font-semibold">Contact</a>
                <a href="{{ route('client.login') }}" class="px-4 py-2 text-slate-900 dark:text-white/80 hover:text-slate-900 dark:hover:text-white font-semibold">Login</a>
        ...
        <div class="max-w-6xl mx-auto px-6 py-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 md:p-8">
                    <div class="text-4xl font-black text-amber-300 tabular-nums mb-2">₹{{ number_format((float) config('packs.celebration.amount_inr', 300), 0) }}</div>
                ...
                <div class="rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-6 md:p-8">
                    <div class="text-4xl font-black text-emerald-300 tabular-nums mb-2">₹{{ number_format((float) config('packs.guest_pay_single.amount_inr', 400), 0) }}</div>
                    <h3 class="text-xl font-semibold mb-3">Guest Contribution — ledger + “pay to you” for <strong>one</strong> event</h3>
                    <ul class="space-y-2 text-slate-900 dark:text-white/80 text-sm leading-relaxed">
                        <li><i class="fas fa-check text-emerald-400 mr-2" aria-hidden="true"></i><strong>₹{{ number_format((float) config('packs.guest_pay_single.amount_inr', 400), 0) }} one-time</strong> → one <strong>credit</strong>; apply it on <strong>Unlock Direct QR</strong> in the app for the event you choose.</li>
                ...
                <div class="cb-marquee-track">
                    ...
                    @foreach(array_merge($cbLogos, $cbLogos) as $logo)
                        <div class="cb-marquee-logo bg-slate-100 border border-slate-200 hover:bg-slate-200 dark:bg-white/5 dark:border-white/10 dark:hover:bg-white/10" role="listitem">
                            ...
                            <span class="cb-marquee-logo-name text-slate-600 dark:text-slate-200">{{ $logo['name'] }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    <section class="bg-indigo-600 text-white">
        <div class="max-w-6xl mx-auto px-6 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
                ...
                <p class="text-indigo-100 mt-2">Free: 1 event · 50 entries · Paid: Celebration ₹{{ number_format((float) config('packs.celebration.amount_inr', 300), 0) }} · Guest Contribution (1 event) ₹{{ number_format((float) config('packs.guest_pay_single.amount_inr', 400), 0) }} · Host Plus Plan ₹{{ number_format((float) config('packs.ledger_duo.amount_inr', 500), 0) }} · Family Plan ₹{{ number_format((float) config('packs.family.amount_inr', 600), 0) }} · Premium Host Plan ₹{{ number_format((float) config('packs.premium_bundle.amount_inr', 700), 0) }} · Professional Plan ₹{{ number_format((float) config('packs.professional.amount_inr', 999), 0) }}. <a href="#faq" class="text-slate-900 dark:text-white font-medium underline decoration-white/50 hover:decoration-white">Read FAQs</a></p>
            </div>
    ...
    <footer class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white/70 border-t border-slate-200 dark:border-white/10">
        <div class="max-w-6xl mx-auto px-6 py-6 text-sm flex flex-col md:flex-row justify-between">
            ...
            <div class="flex flex-wrap gap-4 justify-end">
                ...
                <a href="{{ route('public.contact') }}" class="hover:text-slate-900 dark:hover:text-white">Contact</a>
                <a href="{{ route('public.privacy') }}" class="hover:text-slate-900 dark:hover:text-white">Privacy</a>
                <a href="{{ route('public.terms') }}" class="hover:text-slate-900 dark:hover:text-white">Terms</a>
                <a href="{{ route('client.login') }}" class="hover:text-slate-900 dark:hover:text-white">Login</a>
                <a href="{{ route('client.register') }}" class="hover:text-slate-900 dark:hover:text-white">Register</a>
            </div>
        </div>
    </footer>
    <script>
