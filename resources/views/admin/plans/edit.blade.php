...
@section('title', 'Edit Plan — ' . $pack->name)
...
<div class="p-6 max-w-4xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Manage pricing, description, features, and payment links for <strong>{{ $pack->name }}</strong>.</p>
        </div>
        <span class="inline-flex items-center rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1">
            Level {{ $pack->min_level }}
        </span>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
        <form method="POST" action="{{ route('admin.plans.update', $pack->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $pack->slug }}" disabled
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="1" min="0" name="amount_inr" value="{{ old('amount_inr', $pack->amount_inr) }}" required
                    ...
                    <div class="flex items-center pt-6">
                        <label class="inline-flex items-center gap-2.5 cursor-pointer">
                            <input type="checkbox" name="is_popular" value="1" {{ old('is_popular', $pack->is_popular) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                        class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $pack->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="features" rows="6"
                        class="w-full rounded-xl border border-gray-300 p-4 text-sm font-mono leading-relaxed focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('features', is_array($pack->features) ? implode("\n", $pack->features) : '') }}</textarea>
                </div>
                ...
                <div class="border-t border-gray-100 pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="url" name="live_payment_url" value="{{ old('live_payment_url', $pack->live_payment_url) }}"
                        ...
                        <div>
                            ...
                            <input type="url" name="test_payment_url" value="{{ old('test_payment_url', $pack->test_payment_url) }}"
                                class="w-full rounded-xl border border-gray-300 px-4 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                placeholder="https://rzp.io/rzp/...">
                        </div>
                    </div>
