...
@section('title', 'GPay QR — ' . $event->title)
...
<div class="mb-6">
    ...
    <p class="cb-subtitle">{{ $event->title }} — set how you want to be paid, then show or print the right QR for guests</p>
</div>
...
<div class="mb-6 cb-card p-5 sm:p-6">
    ...
    <form method="POST" action="{{ route('client.gpay.details') }}" enctype="multipart/form-data" class="space-y-4 max-w-lg">
        @csrf
        <input type="hidden" name="event_id" value="{{ $event->id }}">

        <div>
            <label class="cb-label" for="upi_id">UPI ID <span class="text-red-500">*</span></label>
            <input type="text" name="upi_id" id="upi_id" value="{{ old('upi_id', $event->upi_id ?? '') }}"
        ...
        @if($event->gpay_qr_image)
            ...
            <img src="{{ asset('storage/' . $event->gpay_qr_image) }}" alt="Saved GPay QR" class="max-w-[200px] rounded-lg border border-slate-200">
        @endif
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6" id="print-root">
    <div class="cb-card p-5 sm:p-6 bg-white" id="guest-qr-section">
        ...
        <div class="flex flex-col items-center justify-center p-6 sm:p-8 bg-cb-cream/80 rounded-xl mb-4 border border-slate-200 min-h-[280px]">
            @if($hasCustomQr)
                <img id="main-qr"
                     src="{{ asset('storage/' . $event->gpay_qr_image) }}?v={{ $event->updated_at?->timestamp ?? $event->id }}"
            ...
            @elseif($hasUpi)
                <img id="main-qr"
                     src="{{ route('client.gpay.upi-qr', $event->id) }}?t={{ $event->updated_at?->timestamp ?? $event->id }}"
                     alt="UPI QR" class="w-64 h-64 sm:w-72 sm:h-72 object-contain border-4 border-white shadow-lg rounded-lg bg-white p-1">
                <p class="text-xs text-slate-600 mt-3 font-mono">UPI: {{ $event->upi_id }}</p>
            @else
        ...
        <div class="space-y-3">
            @if($hasUpi)
                <div class="flex flex-col sm:flex-row gap-2">
                    @if($hasCustomQr)
                        <a href="{{ asset('storage/' . $event->gpay_qr_image) }}"
                           download="gpay-qr-event-{{ $event->id }}.png"
                    ...
                    @else
                        <a href="{{ route('client.gpay.upi-qr', $event->id) }}?t={{ $event->updated_at?->timestamp ?? 0 }}"
                           download="upi-qr-event-{{ $event->id }}.svg"
                           class="cb-btn cb-btn-gold w-full py-3 rounded-2xl text-center justify-center">
    ...
    <div class="cb-card p-5 sm:p-6 bg-white">
        ...
        <div class="mt-4 p-3 bg-sky-50 border border-sky-200 rounded-lg text-sm text-sky-900 break-all">
            ...
            <code class="text-xs block mt-1">{{ $paymentUrl }}</code>
        </div>
    </div>
</div>
...
<div class="mt-6 cb-card p-5 sm:p-6">
    ...
    <div class="mt-4 flex flex-wrap gap-4 text-sm text-slate-500">
        <div class="bg-slate-50 p-3 rounded-lg flex-1 min-w-[120px]">
            ...
            <p class="text-xl font-bold text-cb-navy">{{ $event->chandlas->count() }}</p>
        </div>
        <div class="bg-slate-50 p-3 rounded-lg flex-1 min-w-[120px]">
            <p class="text-xs text-slate-500">Total amount</p>
            <p class="text-xl font-bold text-cb-gold">₹{{ number_format($event->chandlas->sum('amount'), 2) }}</p>
        </div>
    </div>
</div>
