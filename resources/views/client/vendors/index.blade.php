...
<div class="w-full min-w-0 max-w-5xl mx-auto">
        ...
        <div>
            ...
            @if($event)
                <div class="mt-2 text-sm text-slate-500">
                    Showing vendors for: <span class="font-bold text-cb-navy">{{ $event->title }}</span> 
                    @if($event->venue) ({{ $event->venue }}) @endif
                </div>
            @endif
        </div>
        <div>
            <a href="{{ route('client.vendors.register') }}" class="cb-btn cb-btn-gold text-sm min-h-[2.5rem] justify-center text-center">
    ...
    {{-- Search Filters --}}
    <form method="get" action="{{ route('client.vendors.index') }}" class="cb-card p-3 sm:p-5 mb-5 sm:mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
            ...
            <label class="block min-w-0">
                ...
                <input type="text" name="city" value="{{ request('city') }}" class="mt-0.5 w-full min-w-0 rounded-lg border border-slate-200 px-2.5 sm:px-3 py-2 text-base sm:text-sm" placeholder="Search by city..." autocomplete="address-level2">
            </label>
            <label class="block min-w-0">
                ...
                <select name="price_tier" class="mt-0.5 w-full min-w-0 rounded-lg border border-slate-200 px-2.5 sm:px-3 py-2 text-base sm:text-sm bg-white">
                    ...
                    <option value="budget" {{ request('price_tier') == 'budget' ? 'selected' : '' }}>Budget (₹)</option>
                    <option value="mid" {{ request('price_tier') == 'mid' ? 'selected' : '' }}>Mid-range (₹₹)</option>
                    <option value="premium" {{ request('price_tier') == 'premium' ? 'selected' : '' }}>Premium (₹₹₹)</option>
                </select>
            </label>
        </div>
        <div class="mt-3 sm:mt-4 flex flex-col sm:flex-row flex-wrap gap-2">
            ...
            <a href="{{ route('client.vendors.index', ['event_id' => request('event_id')]) }}" class="cb-btn cb-btn--sm border border-slate-200 w-full sm:w-auto min-h-[2.75rem] justify-center text-center">Clear</a>
        </div>
    </form>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        @forelse($vendors as $v)
            <article class="cb-card overflow-hidden flex flex-col min-w-0 relative group hover:shadow-md transition duration-200 border border-slate-200/80">
                ...
                {{-- Portfolio Thumbnail --}}
                <a href="{{ route('client.vendors.show', ['vendor' => $v->id, 'event_id' => request('event_id')]) }}" class="relative block aspect-[4/3] bg-slate-100 overflow-hidden">
                    @if($v->portfolioImages->isNotEmpty())
                        <img src="{{ asset('storage/' . $v->portfolioImages->first()->image_url) }}" alt="{{ $v->business_name }}" class="h-full w-full object-cover group-hover:scale-105 transition duration-300" loading="lazy" decoding="async">
                    @else
                    ...
                    <div class="absolute bottom-2 right-2">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-bold bg-slate-900/75 text-white backdrop-blur-sm">
                            @if($v->price_tier === 'budget')
                                Budget (₹)
                            @elseif($v->price_tier === 'mid')
                                Mid (₹₹)
                            @else
                                Premium (₹₹₹)
                            @endif
                ...
                <div class="p-3.5 sm:p-4 flex-1 flex flex-col min-w-0">
                    <p class="text-[0.65rem] font-bold uppercase tracking-wider text-amber-600 mb-1 leading-none">{{ $v->category->name }}</p>
                    <h2 class="font-bold text-cb-navy text-base sm:text-lg leading-tight break-words group-hover:text-amber-500 transition duration-150">
                        <a href="{{ route('client.vendors.show', ['vendor' => $v->id, 'event_id' => request('event_id')]) }}">
                            {{ $v->business_name }}
                        </a>
                    </h2>
                    ...
                    @if($v->description)
                        <p class="text-xs text-slate-600 mt-2 break-words line-clamp-2 leading-relaxed">{{ $v->description }}</p>
                    @endif

                    <div class="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between">
                        <a href="{{ route('client.vendors.show', ['vendor' => $v->id, 'event_id' => request('event_id')]) }}" class="text-xs font-semibold text-cb-navy hover:underline inline-flex items-center gap-1">
