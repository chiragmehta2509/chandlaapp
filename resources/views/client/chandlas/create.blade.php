...
@section('title', 'Add Chandla')

@section('content')
<div class="w-full max-w-2xl mx-auto">
    <header class="mb-6 sm:mb-8">
        <a
            href="{{ route('client.chandlas.index') }}"
    ...
    <div class="cb-card overflow-hidden">
        <form method="POST" action="{{ route('client.chandlas.store') }}" enctype="multipart/form-data" class="p-4 sm:p-6 lg:p-8">
            @csrf
                ...
                <section aria-labelledby="chandla-event-heading">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required
                            {{ $eventId ? 'disabled' : '' }}
                            class="cb-field min-h-[48px] w-full {{ $eventId ? '!bg-slate-100 !text-slate-600 cursor-not-allowed' : '' }}"
                        >
                            ...
                            @foreach($events as $event)
                                <option
                                    ...
                                    {{ old('event_id', $eventId) == $event->id ? 'selected' : '' }}
                                >
                ...
                <section aria-labelledby="chandla-giver-heading" class="pt-2 sm:pt-4 border-t border-slate-200/80">
                    ...
                    <div class="space-y-4 sm:space-y-5">
                        <div>
                            ...
                            @if(($lockCashMode ?? false) && isset($giverNameOptions) && $giverNameOptions->count() > 0)
                                <div class="mb-3">
                                    ...
                                    <select id="giver_name_select" class="cb-field min-h-[48px] w-full">
                                        ...
                                        @foreach($giverNameOptions as $giverOption)
                                            <option value="{{ $giverOption }}">{{ $giverOption }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @endif
                            <div class="relative">
                                <input
                                    ...
                                    name="giver_name"
                                    id="giver_name"
                                    value="{{ old('giver_name', $defaultGiverName ?? '') }}"
                                    required
                                    autocomplete="off"
                        ...
                        <div>
                            ...
                            <textarea name="giver_address" id="giver_address" rows="2" class="cb-field min-h-[4.5rem] resize-y w-full" placeholder="Street, area, city…">{{ old('giver_address') }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                            ...
                            <div>
                                ...
                                <input type="email" name="giver_email" id="giver_email" value="{{ old('giver_email') }}" class="cb-field min-h-[48px] w-full" placeholder="Email">
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section aria-labelledby="chandla-type-heading" class="pt-2 sm:pt-4 border-t border-slate-200/80">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-4">
                        <div>
                            ...
                            <select
                                ...
                                id="category"
                                required
                                {{ ($lockCashMode ?? false) ? 'disabled' : '' }}
                                class="cb-field min-h-[48px] w-full {{ ($lockCashMode ?? false) ? '!bg-slate-100 !text-slate-600 cursor-not-allowed' : '' }}"
                            >
                                <option value="">Select type</option>
                                <option value="chandla" {{ old('category', $defaultCategory ?? '') === 'chandla' ? 'selected' : '' }}>Cash</option>
                                <option value="cover" {{ old('category', $defaultCategory ?? '') === 'cover' ? 'selected' : '' }}>Cover</option>
                                <option value="gift" {{ old('category', $defaultCategory ?? '') === 'gift' ? 'selected' : '' }}>Gift</option>
                            </select>
                        </div>
                        ...
                        <div id="payment_method_section">
                            ...
                            >
                                ...
                                <option value="cash" {{ old('payment_method', $defaultPaymentMethod ?? '') === 'cash' ? 'selected' : '' }}>Cash</option>
                                <option value="gpay" {{ old('payment_method', $defaultPaymentMethod ?? '') === 'gpay' ? 'selected' : '' }}>GPay</option>
                                <option value="other" {{ old('payment_method', $defaultPaymentMethod ?? '') === 'other' ? 'selected' : '' }}>N/A</option>
                            </select>
                        </div>
                    </div>
                    @if($lockCashMode ?? false)
                    ...
                    <div class="mb-4 sm:mb-6 p-4 rounded-xl border border-slate-200/90 bg-slate-50/50 space-y-4" id="gpay_fields" style="display: none;">
                        ...
                        <div>
                            ...
                            <input type="text" name="gpay_transaction_id" id="gpay_transaction_id" value="{{ old('gpay_transaction_id') }}" class="cb-field min-h-[48px] w-full" placeholder="UPI ref / transaction ID">
                        </div>
                    ...
                    <div id="gift_fields" style="display: none;" class="mb-4 sm:mb-6 p-4 rounded-xl border border-slate-200/90 bg-slate-50/50 space-y-4">
                        <div>
                            ...
                            <input type="text" name="gift_item_name" id="gift_item_name" value="{{ old('gift_item_name') }}" class="cb-field min-h-[48px] w-full">
                        </div>
                        <div>
                            ...
                            <select name="gift_received" id="gift_received" class="cb-field min-h-[48px] w-full">
                                ...
                                <option value="1" {{ old('gift_received') === '1' ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ old('gift_received') === '0' ? 'selected' : '' }}>No</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div id="cash_notes" style="display: none;" class="mb-4 sm:mb-6 p-4 sm:p-5 rounded-xl border border-slate-200/90 bg-white">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                            <div>
                                ...
                                <input type="number" name="cash_note_1" id="cash_note_1" value="{{ old('cash_note_1', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_2">₹2</label>
                                <input type="number" name="cash_note_2" id="cash_note_2" value="{{ old('cash_note_2', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_5">₹5</label>
                                <input type="number" name="cash_note_5" id="cash_note_5" value="{{ old('cash_note_5', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_10">₹10</label>
                                <input type="number" name="cash_note_10" id="cash_note_10" value="{{ old('cash_note_10', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_20">₹20</label>
                                <input type="number" name="cash_note_20" id="cash_note_20" value="{{ old('cash_note_20', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_50">₹50</label>
                                <input type="number" name="cash_note_50" id="cash_note_50" value="{{ old('cash_note_50', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_100">₹100</label>
                                <input type="number" name="cash_note_100" id="cash_note_100" value="{{ old('cash_note_100', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_200">₹200</label>
                                <input type="number" name="cash_note_200" id="cash_note_200" value="{{ old('cash_note_200', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-600 mb-1.5" for="cash_note_500">₹500</label>
                                <input type="number" name="cash_note_500" id="cash_note_500" value="{{ old('cash_note_500', 0) }}" min="0" class="cb-field cash-note-input min-h-[44px] w-full">
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 pt-2" id="amount_section">
                        <div>
                            ...
                            <input type="number" name="amount" id="amount" value="{{ old('amount') }}" step="0.01" min="0" class="cb-field min-h-[48px] w-full" placeholder="0.00">
                        </div>
                        <div>
                            <label class="cb-label cb-label--classic" for="received_date">Received date *</label>
                            <input type="date" name="received_date" id="received_date" value="{{ old('received_date', date('Y-m-d')) }}" required class="cb-field min-h-[48px] w-full">
                        </div>
                    </div>
                </section>
                ...
                <section aria-labelledby="chandla-extra-heading" class="pt-2 sm:pt-4 border-t border-slate-200/80">
                    ...
                    <div class="space-y-4 sm:space-y-5">
                        <div>
                            ...
                            <input type="text" name="receipt_number" id="receipt_number" value="{{ old('receipt_number') }}" class="cb-field min-h-[48px] w-full">
                        </div>
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3" class="cb-field min-h-[5.5rem] resize-y w-full">{{ old('description') }}</textarea>
                        </div>
                        <div>
                            <label class="cb-label cb-label--classic" for="notes">Notes <span class="font-normal text-slate-400">(optional)</span></label>
                            <textarea name="notes" id="notes" rows="2" class="cb-field min-h-[4rem] resize-y w-full">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </section>
            </div>
            ...
            <div class="mt-8 sm:mt-10 pt-5 sm:pt-6 border-t border-slate-200/80 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
                <a href="{{ route('client.chandlas.index') }}" class="cb-btn cb-btn--ghost w-full sm:w-auto min-h-[48px] px-6 justify-center order-2 sm:order-none">
                    Cancel
                </a>
...
document.addEventListener('DOMContentLoaded', function() {
    const lockCashMode = @json((bool) ($lockCashMode ?? false));
    ...
    const fetchGiverSuggestions = async () => {
        ...
        try {
            const response = await fetch(`{{ route('client.chandlas.search-givers') }}?q=${encodeURIComponent(query)}`);
            if (!response.ok) {
                hideSuggestions();
                return;
            }
    ...
    const fetchPreviousGiver = async () => {
        ...
        try {
            const response = await fetch(`{{ route('client.chandlas.lookup-giver') }}?${params.toString()}`);
            if (!response.ok) {
                hidePreviousGiver();
                return;
            }
