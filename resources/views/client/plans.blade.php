    ...
    <div class="cb-card border border-slate-200/90 bg-white p-5 sm:p-6 rounded-2xl shadow-sm space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-100 pb-3">
            ...
            <span class="text-xs text-slate-500">
                Total Active Items: <strong class="text-slate-800">{{ count($activePacks) > 0 ? count($activePacks) : 1 }}</strong>
            </span>
        </div>

        @if(count($activePacks) === 0)
                ...
                @foreach($activePacks as $pack)
                    <div class="flex items-start gap-4 p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                        <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white border border-slate-200/80 text-base shadow-sm">
                            <i class="fa-solid {{ $pack['icon'] }}" aria-hidden="true"></i>
                        </span>
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                <h3 class="text-sm font-bold text-slate-900 truncate">{{ $pack['name'] }}</h3>
                                <span class="inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider {{ $pack['badge_class'] }}">
                                    {{ $pack['badge'] ?? 'Active' }}
                                </span>
                            </div>
                            <p class="text-xs text-slate-600 mt-1 leading-relaxed">{{ $pack['desc'] }}</p>
                            @if(!empty($pack['date']))
                                <p class="text-[11px] text-slate-400 mt-1.5">Activated: {{ \Carbon\Carbon::parse($pack['date'])->format('d/m/Y') }}</p>
                            @endif
                        </div>
                    </div>
                @endforeach
