...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
    <div class="cb-card cb-card--hero relative overflow-hidden p-5 sm:p-6 lg:col-span-2">
        ...
        <div class="relative">
            <p class="text-xs sm:text-sm font-medium leading-relaxed text-sky-100/95 max-w-2xl">
                Welcome back, <span class="font-semibold text-white">{{ Auth::user()->name }}</span><span class="text-indigo-100/80"> — here's a snapshot of your account.</span>
            </p>
            ...
            <div class="mt-3 min-w-0 sm:mt-4">
                ...
                <div class="mt-3 flex flex-wrap items-center gap-2">
                    <a href="{{ route('client.plans') }}" class="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white shadow-sm backdrop-blur-sm transition hover:bg-white/18 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-300/60">
                        <i class="fa-solid fa-tags text-amber-200 text-[0.75rem]" aria-hidden="true"></i>
                        Plans &amp; pricing
                    </a>
                    <a href="{{ route('client.faq') }}" class="inline-flex items-center gap-2 rounded-full border border-white/15 bg-transparent px-3 py-1.5 text-xs font-semibold text-indigo-50/95 transition hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/25">
            ...
            <div class="mt-4 grid grid-cols-3 gap-2 sm:gap-3">
                <a href="{{ route('client.events.index') }}" class="group relative flex min-h-[7.5rem] flex-col rounded-2xl border border-white/12 bg-slate-950/50 p-3 text-center shadow-sm ring-1 ring-white/[0.06] transition hover:border-amber-300/35 hover:bg-slate-950/70 hover:shadow-lg hover:shadow-black/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/55 sm:min-h-[8rem] sm:p-4">
                    ...
                    <p class="mt-2 text-2xl font-bold tabular-nums leading-none tracking-tight text-white sm:text-[1.85rem]">{{ $stats['total_events'] }}</p>
                    <p class="mt-1.5 text-[0.6rem] font-bold uppercase tracking-[0.14em] text-indigo-100/85">Events</p>
                </a>
                <a href="{{ route('client.chandlas.index') }}" class="group relative flex min-h-[7.5rem] flex-col rounded-2xl border border-white/12 bg-slate-950/50 p-3 text-center shadow-sm ring-1 ring-white/[0.06] transition hover:border-emerald-300/35 hover:bg-slate-950/70 hover:shadow-lg hover:shadow-black/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/55 sm:min-h-[8rem] sm:p-4">
                    ...
                    <p class="mt-2 text-2xl font-bold tabular-nums leading-none tracking-tight text-emerald-100 sm:text-[1.85rem]">{{ $stats['total_entries'] }}</p>
                    <p class="mt-1.5 text-[0.6rem] font-bold uppercase tracking-[0.14em] text-indigo-100/85">Entries</p>
                </a>
                <a href="{{ route('client.contacts.index') }}" class="group relative flex min-h-[7.5rem] flex-col rounded-2xl border border-white/12 bg-slate-950/50 p-3 text-center shadow-sm ring-1 ring-white/[0.06] transition hover:border-violet-300/35 hover:bg-slate-950/70 hover:shadow-lg hover:shadow-black/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-400/55 sm:min-h-[8rem] sm:p-4">
                    ...
                    <p class="mt-2 text-2xl font-bold tabular-nums leading-none tracking-tight text-violet-100 sm:text-[1.85rem]">{{ $stats['total_contacts'] }}</p>
    ...
    {{-- Right-side slot: Full breakdown --}}
            ...
            <div class="cb-card p-3">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <p class="text-lg font-bold text-cb-navy mt-0.5">{{ $stats['total_events'] }}</p>
                    </div>
            ...
            <div class="cb-card p-3">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <p class="text-lg font-bold text-cb-navy mt-0.5">{{ $stats['upcoming_events'] }}</p>
                    </div>
            ...
            <div class="cb-card p-3">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <p class="text-lg font-bold text-cb-navy mt-0.5">{{ $stats['total_contacts'] }}</p>
                    </div>
            ...
            <div class="cb-card p-3">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <p class="text-lg font-bold text-cb-navy mt-0.5">{{ $stats['favorite_contacts'] }}</p>
                    </div>
            ...
            <div class="cb-card p-3">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <p class="text-lg font-bold text-cb-navy mt-0.5">{{ $stats['total_entries'] }}</p>
                    </div>
            ...
            <div class="cb-card p-3">
                <div class="flex items-center justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <p class="text-lg font-bold text-cb-navy mt-0.5">{{ $stats['total_invitations'] }}</p>
                    </div>
...
<div class="mb-6">
    <div class="cb-card p-5 sm:p-6 border-l-4 border-amber-500">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-300 mb-3">
            Total free quota is <span class="font-semibold">{{ $stats['global_free_limit_total'] }}</span> entries across all events until a paid plan is purchased.
        </p>
        <div class="flex flex-wrap gap-6 text-sm">
            <div>Used: <span class="font-semibold text-gray-900 dark:text-white">{{ $stats['global_free_limit_used'] }}</span></div>
            <div>Remaining: <span class="font-semibold text-gray-900 dark:text-white">{{ $stats['global_free_limit_remaining'] }}</span></div>
        </div>
        <div class="mt-4 flex flex-wrap items-center gap-2 sm:gap-3">
            @if($stats['show_free_limit_download'])
                <a href="{{ route('client.chandlas.free-limit.download') }}" class="cb-btn cb-btn-navy cb-btn--sm inline-flex items-center">
                    <i class="fas fa-download mr-2"></i>Download First 50 Entries
                </a>
            @endif
            @if($upgradeEvent)
                <a href="{{ route('client.events.plan.payment', ['id' => $upgradeEvent->id, 'plan' => 'unlimited']) }}" class="inline-flex items-center px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold">
                    <i class="fas fa-arrow-up mr-2"></i>Upgrade Plan
                </a>
            @else
                <a href="{{ route('client.events.create') }}" class="inline-flex items-center px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold">
...
<div class="mb-6 sm:mb-8">
    <div class="cb-card p-5 sm:p-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4 mb-5 sm:mb-6">
            ...
            <a href="{{ route('client.events.index') }}" class="cb-link text-sm font-medium whitespace-nowrap shrink-0 self-start sm:self-auto inline-flex items-center gap-1">
        ...
        <div
            ...
            @endif
            class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 min-h-0 {{ $quickListScrollable ? 'max-h-[min(70vh,32rem)] overflow-y-auto overscroll-y-contain touch-pan-y rounded-xl border border-slate-200/80 bg-slate-50/50 p-2 sm:p-3 [-webkit-overflow-scrolling:touch]' : '' }}"
        >
            @foreach($dashboardQuickEvents as $event)
                <div class="group flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-gradient-to-br from-white to-slate-50/70 p-4 min-w-0 shadow-sm hover:shadow-md hover:border-amber-300/70 transition-all duration-200">
                    <div class="min-w-0 flex-1">
                        <p class="font-semibold text-cb-navy leading-snug break-words">{{ $event->title }}</p>
                        <div class="mt-2 flex flex-col gap-1.5 text-xs text-slate-600 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-3 sm:gap-y-1">
                            <span class="inline-flex items-start gap-1 shrink-0"><i class="fas fa-calendar-day text-cb-gold/90 mt-0.5 shrink-0" aria-hidden="true"></i>{{ $event->event_date->format('d/m/Y') }}</span>
                            @if($event->venue)
                                <span class="inline-flex items-start gap-1 break-words"><i class="fas fa-map-marker-alt text-cb-gold/90 mt-0.5 shrink-0" aria-hidden="true"></i>{{ $event->venue }}</span>
                            @endif
                        </div>
                    </div>
...
<div class="mb-6 sm:mb-8">
    <div class="cb-card p-5 sm:p-6 border border-dashed border-slate-300/90 bg-slate-50/80">
        ...
        <a href="{{ route('client.events.create') }}" class="inline-flex items-center mt-4 cb-btn cb-btn-gold cb-btn--sm">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            ...
            <div class="min-w-[220px]">
                ...
                <select id="eventChartSelect" class="cb-field text-sm">
                    ...
                    @foreach($eventBreakdown as $eventRow)
                        <option value="{{ $eventRow['id'] }}">{{ $eventRow['title'] }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <div class="cb-card p-5 sm:p-6 lg:col-span-1">
        ...
        <div class="grid grid-cols-2 gap-3 sm:gap-4">
            @canEdit
            <a href="{{ route('client.chandlas.create') }}" class="cb-qaction cb-qaction--gold">
                <i class="fas fa-plus"></i>
                <span>Entry</span>
            </a>
            <a href="{{ route('client.events.create') }}" class="cb-qaction cb-qaction--blue">
                <i class="fas fa-calendar-plus"></i>
                <span>Event</span>
            </a>
            <a href="{{ route('client.contacts.create') }}" class="cb-qaction cb-qaction--teal">
                <i class="fas fa-user-plus"></i>
                <span>Contact</span>
            </a>
            @endcanEdit
            <a href="{{ route('client.events.index') }}" class="cb-qaction cb-qaction--violet">
                <i class="fas fa-calendar"></i>
                <span>All events</span>
            </a>
            <a href="{{ route('client.chandlas.index') }}" class="cb-qaction cb-qaction--violet">
                <i class="fas fa-list"></i>
                <span>Ledger</span>
            </a>
            <a href="{{ route('client.contacts.index') }}" class="cb-qaction cb-qaction--violet">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                @forelse($upcoming_events as $event)
                    <div class="border-b border-gray-200 pb-4 last:border-0">
                        ...
                        <p class="text-sm text-gray-600 mt-1">
                            <i class="fas fa-calendar mr-2"></i>{{ $event->event_date->format('d/m/Y') }}
                            @if($event->venue)
                                <span class="ml-4"><i class="fas fa-map-marker-alt mr-2"></i>{{ $event->venue }}</span>
                            @endif
                        </p>
                        <a href="{{ route('client.events.show', $event->id) }}" class="cb-link text-sm mt-2 inline-block">
                            View Details →
                        </a>
                    </div>
                @empty
    ...
    <div class="cb-card overflow-hidden">
        ...
        <div class="p-4 sm:p-6">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="flex-shrink-0">
                            <div class="h-10 w-10 rounded-full bg-amber-100 flex items-center justify-center ring-2 ring-amber-200/60">
                                <span class="text-amber-800 font-semibold">{{ substr($contact->name, 0, 1) }}</span>
                            </div>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate">{{ $contact->name }}</p>
                            <p class="text-sm text-gray-500 truncate">{{ $contact->phone ?? $contact->email ?? 'N/A' }}</p>
                        </div>
                        <div class="flex-shrink-0">
            ...
            <div class="mt-4">
                <a href="{{ route('client.contacts.index') }}" class="cb-link text-sm font-medium">View all contacts →</a>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-5">
            ...
            <div class="shrink-0 text-left sm:text-right">
                ...
                <div class="inline-flex items-center gap-2 rounded-xl border border-amber-200/80 bg-gradient-to-br from-amber-50 to-amber-100/50 px-3 py-2.5 min-h-[2.75rem]">
                    ...
                    <span class="text-lg font-bold text-cb-gold leading-none">{{ $stats['free_event_credits'] }}</span>
                    <span class="text-xs text-slate-600">available</span>
                </div>
                @if($stats['free_event_credits'] > 0)
                <div class="mt-2 text-left sm:text-right">
                    <a href="{{ route('client.events.create', ['redeem' => 'true']) }}" class="text-xs font-semibold text-cb-gold hover:text-amber-700 underline decoration-amber-300 underline-offset-2">Redeem for Event →</a>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-4 lg:items-end w-full">
            <div class="w-full lg:w-[35%] min-w-0">
                ...
                <div class="flex items-stretch gap-2">
                    <div id="referralCodeText" class="flex-1 min-w-0 rounded-xl px-3 py-2.5 font-mono text-sm text-cb-navy bg-[#F0EDE6] border border-cb-navy/10 break-all text-left flex items-center dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700">{{ $user->referral_code }}</div>
                    <button type="button"
            ...
            <div class="w-full lg:w-[65%] min-w-0">
                ...
                <div class="flex items-stretch gap-2">
                    <div id="referralLinkText" class="flex-1 min-w-0 rounded-xl px-3 py-2.5 font-mono text-xs sm:text-sm text-cb-navy bg-[#F0EDE6] border border-cb-navy/10 break-all text-left flex items-center dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700">{{ route('client.register', ['ref' => $user->referral_code]) }}</div>
                    <button type="button"
                            class="js-copy-btn shrink-0 inline-flex items-center justify-center gap-1.5 rounded-xl border border-cb-navy/15 bg-white text-cb-navy px-4 text-xs font-semibold hover:bg-cb-navy hover:text-white hover:border-cb-navy transition focus:outline-none focus-visible:ring-2 focus-visible:ring-cb-gold/40"
...
<script>
    ...
    const eventBreakdown = @json($eventBreakdown);
    const allTotals = @json($allTotals);
