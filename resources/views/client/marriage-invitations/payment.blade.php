...
@if ($errors->has('pay'))
    <div class="mb-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{{ $errors->first('pay') }}</div>
@endif

{{-- Dynamic Razorpay Checkout --}}
<div class="cb-card p-6 sm:p-8 mb-8 border-2 border-emerald-200/80 bg-gradient-to-br from-emerald-50/90 via-white to-white max-w-2xl">
    ...
    <button id="rzp-pay-btn" type="button"
        ...
        Pay ₹{{ number_format($amount, 0) }} — Unlock Now
    </button>
    <p class="text-xs text-slate-500 mt-4">Powered by Razorpay · SSL secured</p>
</div>

{{-- Hidden form to POST verification data after Razorpay success --}}
<form id="rzp-verify-form" method="POST"
    action="{{ route('client.marriage-invitations.payment.razorpay.verify', $invitation->id) }}"
    style="display:none;">
    @csrf
...
document.getElementById('rzp-pay-btn').addEventListener('click', function () {
    ...
    fetch('{{ route('client.marriage-invitations.payment.razorpay.order', $invitation->id) }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.message && !data.order_id) {
            ...
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-lock-open text-xs mr-2"></i> Pay ₹{{ number_format($amount, 0) }} — Unlock Now';
            return;
        }
