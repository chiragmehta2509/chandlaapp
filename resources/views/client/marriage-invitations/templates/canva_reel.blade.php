...
<head>
    ...
    <title>Video — {{ $d['groom_name'] ?? '' }} & {{ $d['bride_name'] ?? '' }}</title>
...
<div class="capture-root">
    ...
    <p class="cr-eyebrow">{{ $d['tagline'] ?? "We're getting married" }}</p>
    <div class="cr-photo-shell">
        <div class="cr-photo">
            @if($coupleImageOk && $imgSrc)
                <img src="{{ $imgSrc }}" alt="" width="400" height="400" @if(!empty($videoExportScript)) loading="eager" decoding="sync" fetchpriority="high" @else loading="lazy" @endif>
            @else
                <div class="cr-photo-ph">Add your photo in Edit to complete this look.</div>
            @endif
        </div>
    </div>
    <div class="cr-names">{{ $d['groom_name'] ?? '' }}</div>
    <p class="cr-amp">&amp;</p>
    <div class="cr-names">{{ $d['bride_name'] ?? '' }}</div>
    <div class="cr-pills">
        <div class="cr-pill">
            <strong>When</strong>
            <span>{{ $dateLine ?: '—' }}@if(!empty($d['wedding_time']))<br>{{ \App\Support\MarriageInvitationCard::formatWeddingTimeForDisplay($d['wedding_time'] ?? null) }}@endif</span>
        </div>
        <div class="cr-pill">
            <strong>Where</strong>
            <span><strong style="font-weight:700">{{ $d['venue_name'] ?? '' }}</strong>@if(!empty($d['venue_address']))<br><span style="opacity:.9">{{ $d['venue_address'] }}</span>@endif</span>
        </div>
    </div>
...
@endif
@include('client.marriage-invitations.partials.export-video-script')
</body>
</html>
