...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        @if(!empty($pngExportScript)) body.png-lav { background: #ebe7f2 !important; } @endif
        .capture-root {
            max-width: 420px;
            margin: 0 auto;
            background: #f4f4f7;
...
<div class="capture-root">
    ...
    <p class="ld-sub">{{ $d['tagline'] ?? 'A dreamy celebration' }}</p>
    <div class="ld-photo-shell">
        <div class="ld-photo">
            @if($coupleImageOk && $imgSrc)
                <img src="{{ $imgSrc }}" alt="" width="222" height="296" @if(!empty($pngExportScript)) loading="eager" decoding="sync" fetchpriority="high" @else loading="lazy" @endif>
            @else
                <div class="ld-photo-ph">Your photo appears here when you add one in Edit.</div>
            @endif
        </div>
    </div>
    <h1 class="ld-name">{{ $d['groom_name'] ?? '' }}</h1>
    <p class="ld-amp">&amp;</p>
    <h1 class="ld-name">{{ $d['bride_name'] ?? '' }}</h1>
    ...
    <div class="ld-pill">
        <h2>When</h2>
        <p>{{ $dateLine ?: '—' }}@if(!empty($d['wedding_time']))<br><span style="font-weight:600">{{ \App\Support\MarriageInvitationCard::formatWeddingTimeForDisplay($d['wedding_time'] ?? null) }}</span>@endif</p>
    </div>
    <div class="ld-pill">
        <h2>Where</h2>
        <p><strong>{{ $d['venue_name'] ?? '' }}</strong></p>
        @if(!empty($d['venue_address']))
            <p class="ld-venue-line2">{{ $d['venue_address'] }}</p>
        @endif
    </div>
    @include('client.marriage-invitations.partials.schedule-block', ['d' => $d, 'wrapClass' => 'ld-sched', 'titleClass' => '', 'heading' => 'Schedule', 'rowClass' => 'ld-srow', 'titleCellClass' => 'ld-st', 'dotsClass' => 'ld-dots', 'whenClass' => 'ld-sw'])
    @if(!empty($d['rsvp_contact']))<p class="ld-foot">Contact: {{ $d['rsvp_contact'] }}</p>@endif
