...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6 sm:mb-8 rounded-2xl border overflow-hidden
        ...
        @else border-slate-200 bg-white @endif" id="mi-status-banner">
        <div class="flex gap-4 p-4 sm:p-5">
            <div class="shrink-0 w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-lg
                @if($invitation->isUnlocked()) bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400
                @elseif($invitation->hasPendingPayment()) bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-400
                @elseif($invitation->exportsUnlockedForUser()) bg-sky-100 dark:bg-sky-900/50 text-sky-700 dark:text-sky-400
                @else bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 @endif" aria-hidden="true">
                @if($invitation->isUnlocked())
            ...
            <div class="min-w-0 flex-1">
                ...
                @if($invitation->isUnlocked())
                    ...
                    <p class="text-xs text-slate-600 dark:text-slate-400 mt-1">Paid {{ $invitation->paid_at?->format('d/m/Y g:i A') }}</p>
                @elseif($invitation->hasPendingPayment())
                    ...
                    <div class="p-4 sm:p-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            @foreach($templates as $key => $t)
                                <div class="template-card rounded-xl border p-4 flex flex-col gap-3 transition-all duration-200 cursor-pointer {{ $key === $firstKey ? 'border-amber-500 dark:border-amber-600 bg-amber-50/40 dark:bg-amber-900/20 ring-2 ring-amber-400/30' : 'border-slate-200 dark:border-slate-600 bg-slate-50/50 dark:bg-slate-700/50 hover:border-amber-300/60 hover:bg-amber-50/20 dark:hover:border-amber-600/60 dark:hover:bg-amber-900/20' }}" data-layout="{{ $key }}" onclick="updateLivePreview('{{ $key }}', '{{ addslashes($t['name']) }}')">
    ...
    @if($hasSchedule)
        <div class="mt-6 {{ $invitation->exportsUnlockedForUser() ? 'w-full' : 'max-w-2xl mx-auto' }}">
            <div class="rounded-2xl border border-slate-200/90 bg-white dark:bg-slate-800 dark:border-slate-700 shadow-sm overflow-hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach($cardDisplay['schedule_events'] as $ev)
                            ...
                            <div class="flex items-center gap-4 p-4 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-900/20 hover:border-amber-200/80 transition-colors">
                                ...
                                <div class="min-w-0 flex-1">
                                    <h4 class="font-bold text-cb-navy dark:text-white text-base truncate">{{ $ev['title'] }}</h4>
    ...
    <div class="mt-6 cb-card p-5 sm:p-6 shadow-sm" id="mi-video-tips">
        ...
        <a href="{{ route('client.marriage-invitations.export.video', $invitation->id) }}" target="_blank" rel="noopener" class="cb-btn cb-btn--gold w-full justify-center mt-5 py-3 text-base shadow-md">
    ...
    <div class="mt-8 rounded-2xl border border-slate-200/90 dark:border-slate-700 bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-800/80 shadow-sm overflow-hidden w-full relative">
        ...
        <div class="px-6 py-5 border-b border-slate-100/80 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <a href="{{ route('client.marriage-invitations.edit', $invitation->id) }}" class="cb-btn cb-btn--ghost cb-btn--sm self-start sm:self-auto border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 shadow-sm">
    ...
    @if(!$invitation->exportsUnlockedForUser())
        ...
        @else
            ...
            <div class="rounded-2xl border border-amber-200/80 dark:border-amber-800/60 bg-gradient-to-br from-amber-50/60 to-white dark:from-amber-900/20 dark:to-slate-800/60 p-5 sm:p-6 mb-6">
                ...
                <p class="text-sm text-slate-700 dark:text-slate-300 mb-4 max-w-2xl">Pay the <strong>₹{{ $packPriceFmt }} celebration pack</strong> on Razorpay to unlock everything — or <strong>₹{{ number_format((float) config('marriage_invitations.amount', 300), 0) }}</strong> for this invitation card only (Razorpay).</p>
