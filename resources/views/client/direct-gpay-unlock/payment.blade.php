...
<div class="max-w-3xl mx-auto space-y-6 pb-8">
    ...
    <div>
        ...
        <div class="mt-2 rounded-xl bg-slate-50 border border-slate-150 p-4">
            ...
            <p class="text-base font-extrabold text-cb-navy mt-0.5">{{ $event->title }}</p>
        </div>
    ...
    {{-- Alert --}}
    @if ($errors->has('pack'))
        <div class="cb-alert cb-alert--error" role="status">{{ $errors->first('pack') }}</div>
    @endif
        ...
        @if(($guestPayPackCredits ?? 0) > 0)
            <div class="cb-card p-5 sm:p-6 border-2 border-amber-200 bg-amber-50/80 rounded-2xl shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-xs text-slate-700">You have <strong>{{ (int) $guestPayPackCredits }}</strong> unused credit{{ (int) $guestPayPackCredits === 1 ? '' : 's' }} in your account. Applying this credit will unlock this event instantly.</p>
                </div>
                <form method="POST" action="{{ route('client.events.direct-gpay-unlock.redeem-guest-pay-pack', $event) }}" class="shrink-0 w-full sm:w-auto">
                    @csrf
