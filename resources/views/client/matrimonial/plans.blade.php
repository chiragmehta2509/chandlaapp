            ...
            @foreach($plans as $key => $def)
                <div class="cb-card p-4 sm:p-6 flex flex-col min-w-0">
                    ...
                    <button
                        ...
                        data-plan="{{ $key }}"
                        data-amount="{{ number_format($def['price_inr'], 0) }}"
                        data-name="{{ $def['label'] }}"
                    >
                        <i class="fas fa-lock" aria-hidden="true"></i> Pay ₹{{ number_format($def['price_inr'], 0) }} with Razorpay
                    </button>
                </div>
            @endforeach
        </div>

        <form id="rzp-verify-form" method="POST" action="{{ route('client.matrimonial.plans.verify') }}" class="hidden">
            @csrf
...
<script>
(function () {
    var orderUrl = @json(route('client.matrimonial.plans.order'));
    var token = @json(csrf_token());
    var userName = @json($user->name ?? 'User');
    var userEmail = @json($user->email ?? '');
    var userPhone = @json($user->phone ?? '');
