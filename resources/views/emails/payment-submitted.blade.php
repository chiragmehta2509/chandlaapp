...
<head>
    ...
    <title>Payment recorded — {{ $event->title }}</title>
</head>
<body style="margin:0;padding:0;background-color:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;-webkit-font-smoothing:antialiased;">
    <span style="display:none !important;visibility:hidden;mso-hide:all;font-size:1px;line-height:1px;color:#f1f5f9;max-height:0;max-width:0;opacity:0;overflow:hidden;">
        {{ $chandla->giver_name }} · INR {{ number_format($chandla->amount, 2) }} · {{ $event->title }} — review in your dashboard.
    </span>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;margin:0 auto;">
                    <tr>
                        <td style="background:linear-gradient(135deg,#1e1b4b 0%,#312e81 45%,#4c1d95 100%);background-color:#312e81;border-radius:12px 12px 0 0;padding:24px 28px;text-align:center;">
                            @php
                                $logoPng = public_path('images/chandla-logo.png');
                                $logoSvg = public_path('images/chandla-favicon.png');
                            @endphp
                            @if(isset($message) && file_exists($logoPng))
                                <img src="{{ $message->embed($logoPng) }}" width="140" height="auto" alt="Chandla Book" style="display:block;margin:0 auto 10px auto;max-width:140px;height:auto;border:0;">
                            @elseif(file_exists($logoPng))
                                <img src="{{ url('images/chandla-logo.png') }}" width="140" height="auto" alt="Chandla Book" style="display:block;margin:0 auto 10px auto;max-width:140px;height:auto;border:0;">
                            @elseif(isset($message) && file_exists($logoSvg))
                                <img src="{{ $message->embed($logoSvg) }}" width="48" height="48" alt="" style="display:block;margin:0 auto 10px auto;border:0;">
                            @elseif(file_exists($logoSvg))
                                <img src="{{ url('images/chandla-favicon.png') }}" width="48" height="48" alt="" style="display:block;margin:0 auto 10px auto;border:0;">
                            @endif
                    ...
                    <tr>
                        <td style="background-color:#ffffff;padding:32px 28px 24px 28px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">
                            ...
                            <p style="margin:0 0 28px 0;font-size:16px;line-height:1.6;color:#475569;">
                                Someone submitted a payment for <strong style="color:#1e1b4b;">{{ $event->title }}</strong>. Review the details below and confirm the entry in your ledger if needed.
                            </p>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#f8fafc;border-radius:10px;border:1px solid #e2e8f0;overflow:hidden;">
                                <tr>
                                    <td style="padding:18px 20px;border-bottom:1px solid #e2e8f0;">
                                        ...
                                        <p style="margin:6px 0 0 0;font-size:28px;font-weight:700;color:#059669;letter-spacing:-0.02em;">₹{{ number_format($chandla->amount, 2) }}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding:0;">
                                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                                            <tr>
                                                ...
                                                <td style="padding:14px 20px;border-bottom:1px solid #e2e8f0;vertical-align:top;">
                                                    <span style="font-size:15px;color:#0f172a;font-weight:600;">{{ $event->title }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding:14px 20px;border-bottom:1px solid #e2e8f0;vertical-align:top;">
                                                ...
                                                <td style="padding:14px 20px;border-bottom:1px solid #e2e8f0;vertical-align:top;">
                                                    <span style="font-size:15px;color:#1e293b;">{{ $chandla->giver_name }}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding:14px 20px;border-bottom:1px solid #e2e8f0;vertical-align:top;">
                                                ...
                                                <td style="padding:14px 20px;border-bottom:1px solid #e2e8f0;vertical-align:top;">
                                                    <span style="font-size:15px;color:#1e293b;">{{ $chandla->received_date->format('F j, Y') }}</span>
                                                </td>
                                            </tr>
                                            @if(!empty($chandla->gpay_transaction_id))
                                            <tr>
                                                <td style="padding:14px 20px;vertical-align:top;">
                                                    <span style="font-size:12px;color:#64748b;font-weight:600;">Reference</span>
                                                </td>
                                                <td style="padding:14px 20px;vertical-align:top;">
                                                    <span style="font-size:14px;color:#0f172a;font-family:ui-monospace,Consolas,monospace;word-break:break-all;">{{ $chandla->gpay_transaction_id }}</span>
                                                </td>
                                            </tr>
                                            @endif
                                        </table>
                            ...
                            @php $chandlaUrl = route('client.chandlas.show', $chandla); @endphp
                            <table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" style="margin:28px auto 0 auto;">
                                <tr>
                                    <td style="border-radius:8px;background-color:#4f46e5;">
                                        <a href="{{ $chandlaUrl }}" target="_blank" style="font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;display:inline-block;padding:14px 28px;border-radius:8px;">View entry in dashboard</a>
                                    </td>
                                </tr>
                            </table>
                            <p style="margin:18px 0 0 0;font-size:13px;color:#64748b;line-height:1.5;">
                                Or open: <a href="{{ $chandlaUrl }}" style="color:#4f46e5;word-break:break-all;">{{ $chandlaUrl }}</a>
                            </p>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td style="padding:18px 16px 8px 16px;text-align:center;">
                            <p style="margin:0;font-size:11px;color:#94a3b8;">&copy; {{ date('Y') }} Chandla Book</p>
                        </td>
                    </tr>
                </table>
            </td>
