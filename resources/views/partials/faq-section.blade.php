...
@endphp
<section id="faq" class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white dark:text-white scroll-mt-20 {{ ($faqFlush ?? false) ? 'border-t border-slate-200/80 dark:border-slate-700/80' : 'border border-slate-200/80 dark:border-slate-700/80 rounded-xl' }}" aria-labelledby="faq-heading">
    <div class="max-w-3xl mx-auto px-6 py-12">
        ...
        <dl class="space-y-4">
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                <dt class="font-semibold text-slate-900 dark:text-white">What’s in the ₹{{ number_format((float) config('packs.celebration.amount_inr', 300), 0) }} Celebration Plan?</dt>
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                <dt class="font-semibold text-slate-900 dark:text-white">What is the Guest Contribution (₹{{ number_format((float) config('packs.guest_pay_single.amount_inr', 400), 0) }})?</dt>
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                <dt class="font-semibold text-slate-900 dark:text-white">What is the Host Plus Plan (₹{{ number_format((float) config('packs.ledger_duo.amount_inr', 500), 0) }})?</dt>
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                <dt class="font-semibold text-slate-900 dark:text-white">What does the Premium Host Plan (₹{{ number_format((float) config('packs.premium_bundle.amount_inr', 700), 0) }}) include?</dt>
                <dd class="mt-2 text-slate-600 dark:text-slate-300 text-sm leading-relaxed">It includes <strong>everything in the Celebration Plan</strong>, <strong>3 events</strong> on your account, <strong>account-wide unlimited chandla</strong> (same class as Host Plus Plan for the ledger), and <strong>full event PDF</strong> workflows. You can still buy <strong>Guest Contribution</strong> credits or use the in-app Direct GPay unlock (currently ₹{{ number_format((float) config('services.direct_gpay_unlock.amount', 400), 0) }} per event) when you need pay-to-you QR on extra events.</dd>
            </div>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                <dt class="font-semibold text-slate-900 dark:text-white">What is the Family Plan (₹{{ number_format((float) config('packs.family.amount_inr', 600), 0) }})?</dt>
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                <dt class="font-semibold text-slate-900 dark:text-white">What does the Professional Plan (₹{{ number_format((float) config('packs.professional.amount_inr', 999), 0) }}) offer?</dt>
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                ...
                <dd class="mt-2 text-slate-600 dark:text-slate-300 text-sm leading-relaxed">For an event you add <strong>your UPI or an uploaded static QR</strong>. We generate a <strong>dedicated scannable pay QR and link</strong> so <strong>guests pay you directly</strong> (they choose the amount), and the app records the line in your ledger. Unlock either with the <strong>in‑app per‑event fee</strong> or a <strong>₹{{ number_format((float) config('packs.guest_pay_single.amount_inr', 400), 0) }} Guest Contribution</strong> credit — see the <a href="{{ $faqDirectGpayHref }}" class="text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Direct GPay section on the plans page</a>.</dd>
            </div>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
                <dt class="font-semibold text-slate-900 dark:text-white">When do I get a referral reward?</dt>
                <dd class="mt-2 text-slate-600 dark:text-slate-300 text-sm leading-relaxed">When your referred contact <strong>completes a qualifying payment</strong> in the app, you can receive a <strong>free event with unlimited entries + full PDF</strong> — as described in the <a href="{{ $faqReferHref }}" class="text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Refer &amp; earn</a> section on our website.</dd>
            </div>
        </dl>
    </div>
</section>
