...
<head>
    ...
    <title>Thank you — <?php echo e($event->title); ?></title>
...
<body class="min-h-screen relative overflow-x-hidden text-slate-800">
    ...
    <div class="relative z-10 max-w-md mx-auto px-4 sm:px-5 py-10 sm:py-14">
        <header class="text-center mb-6">
            <a href="<?php echo e(route('public.home')); ?>" class="inline-flex flex-col items-center gap-2 no-underline">
        ...
        <div class="rounded-2xl bg-white/95 backdrop-blur-sm shadow-2xl ring-1 ring-white/50 p-6 sm:p-8 text-center">
            ...
            <p class="text-slate-600 mt-2 text-base">Your entry is recorded for <strong class="text-slate-800"><?php echo e($event->title); ?></strong>.</p>

            <dl class="mt-8 text-left text-sm space-y-3 border-t border-slate-100 pt-6">
                <div class="flex justify-between gap-4 border-b border-slate-50 pb-2"><dt class="text-slate-500">Name</dt><dd class="font-medium text-slate-800 text-right"><?php echo e($chandla->giver_name); ?></dd></div>
                <div class="flex justify-between gap-4 border-b border-slate-50 pb-2"><dt class="text-slate-500">Amount</dt><dd class="font-bold text-emerald-800 text-right tabular-nums">₹<?php echo e(number_format($chandla->amount, 2)); ?></dd></div>
                <div class="flex justify-between gap-4 border-b border-slate-50 pb-2"><dt class="text-slate-500">Village</dt><dd class="font-medium text-slate-800 text-right"><?php echo e($chandla->giver_address); ?></dd></div>
                <div class="flex justify-between gap-4 border-b border-slate-50 pb-2"><dt class="text-slate-500">Phone</dt><dd class="font-medium text-slate-800 text-right"><?php echo e($chandla->giver_phone); ?></dd></div>
                <div class="flex justify-between gap-4"><dt class="text-slate-500">Category</dt><dd class="font-medium text-slate-800 text-right"><?php echo e($chandla->category); ?></dd></div>
            </dl>

            <?php if(!($emailStatus['organizer_sent'] ?? false)): ?>
        ...
        <p class="text-center text-indigo-200/50 text-xs mt-8">
            <a href="<?php echo e(route('public.home')); ?>" class="text-indigo-200/80 hover:text-white transition">Chandla Book</a>
        </p>
    </div>
</body>
</html>
<?php /**PATH /home/chandlabook/public_html/resources/views/public/direct-gpay-success.blade.php ENDPATH**/ ?>