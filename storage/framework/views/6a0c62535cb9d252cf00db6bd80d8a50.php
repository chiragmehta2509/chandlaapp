...
<head>
    ...
    <title>Pay ₹<?php echo e($amount); ?> — <?php echo e($event->title); ?></title>
...
<body class="min-h-screen relative overflow-x-hidden text-slate-800">
    ...
    <div class="relative z-10 max-w-md mx-auto px-4 sm:px-5 py-8 sm:py-12">
        ...
        <div class="rounded-2xl bg-white/95 backdrop-blur-sm shadow-2xl shadow-slate-900/50 ring-1 ring-white/50 overflow-hidden text-center p-5 sm:p-7">
            ...
            <h1 class="text-lg sm:text-xl font-bold text-slate-900 leading-snug px-1"><?php echo e($event->title); ?></h1>
            <p class="text-slate-600 text-sm mt-1"><?php echo e($data['giver_name']); ?></p>
            <div class="mt-4 inline-flex items-baseline justify-center gap-0.5 rounded-2xl bg-gradient-to-br from-emerald-50 to-sky-50 border border-emerald-100/80 px-6 py-3">
                ...
                <span class="text-3xl sm:text-4xl font-extrabold text-emerald-800 tabular-nums tracking-tight"><?php echo e($amount); ?></span>
            </div>
            ...
            <form method="POST" action="<?php echo e(route('public.direct-gpay.complete', ['event' => $event->id])); ?>" class="mt-4">
                <?php echo csrf_field(); ?>
                <button type="submit"
            ...
            <a href="<?php echo e(route('public.direct-gpay', ['event' => $event->id])); ?>"
               class="inline-block mt-5 text-sm text-slate-500 hover:text-indigo-600 transition">Start over</a>
        </div>
    </div>
</body>
</html>
<?php /**PATH /home/chandlabook/public_html/resources/views/public/direct-gpay-pay.blade.php ENDPATH**/ ?>