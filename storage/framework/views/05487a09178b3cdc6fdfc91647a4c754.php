...
<div class="max-w-4xl mx-auto">
    ...
    <div class="cb-card cb-card--hero relative overflow-hidden p-5 sm:p-7 mb-5 sm:mb-6">
        ...
        <div class="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-4 sm:gap-5 min-w-0">
                <div class="relative shrink-0">
                    <div class="h-16 w-16 sm:h-20 sm:w-20 rounded-full bg-gradient-to-br from-amber-200 via-amber-300 to-amber-400 flex items-center justify-center ring-4 ring-white/20 shadow-lg shadow-black/30">
                        <span class="text-amber-900 font-bold text-2xl sm:text-3xl"><?php echo e($initial); ?></span>
                    </div>
                ...
                <div class="min-w-0">
                    <h1 class="text-xl sm:text-2xl lg:text-[1.65rem] font-bold tracking-tight text-white leading-tight break-words"><?php echo e($contact->name); ?></h1>
                    <?php if($contact->relationship): ?>
                        <p class="mt-1 text-xs sm:text-sm font-medium text-amber-200/95">
                            <i class="fas fa-user-tag mr-1.5 opacity-80" aria-hidden="true"></i><?php echo e($contact->relationship); ?>
            ...
            <div class="flex flex-wrap items-center gap-2 shrink-0">
                <a href="<?php echo e(route('client.contacts.edit', $contact->id)); ?>"
                ...
                <form action="<?php echo e(route('client.contacts.toggle-favorite', $contact->id)); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                    <button type="submit"
                            class="inline-flex items-center gap-2 rounded-full border border-white/20 <?php echo e($contact->is_favorite ? 'bg-amber-400 text-amber-950 hover:bg-amber-300' : 'bg-white/10 text-white hover:bg-white/20'); ?> px-3.5 py-2 text-xs font-semibold shadow-sm backdrop-blur-sm transition focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-300/60">
                        <i class="fa-<?php echo e($contact->is_favorite ? 'solid' : 'regular'); ?> fa-star text-[0.75rem]" aria-hidden="true"></i>
                        <?php echo e($contact->is_favorite ? 'Favorited' : 'Favorite'); ?>

                    </button>
                </form>
                <?php if (\Illuminate\Support\Facades\Blade::check('canDelete')): ?>
                <form action="<?php echo e(route('client.contacts.destroy', $contact->id)); ?>" method="POST" class="inline"
    ...
    <?php if($contact->phone || $contact->email): ?>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 mb-5 sm:mb-6">
            <?php if($contact->phone): ?>
                ...
                <?php if($phoneDigits !== ''): ?>
                    <a href="https://wa.me/<?php echo e($phoneDigits); ?>" target="_blank" rel="noopener"
                ...
                <?php endif; ?>
                <a href="sms:<?php echo e($contact->phone); ?>"
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                ...
                <div class="flex items-start gap-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3.5 min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <dd class="mt-0.5 text-sm font-semibold text-cb-navy break-all">
                            <a href="tel:<?php echo e($contact->phone); ?>" class="hover:underline"><?php echo e($contact->phone); ?></a>
                        </dd>
                    </div>
                    <button type="button" data-copy="<?php echo e($contact->phone); ?>"
            ...
            <?php if($contact->email): ?>
                <div class="flex items-start gap-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3.5 min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <dd class="mt-0.5 text-sm font-semibold text-cb-navy break-all">
                            <a href="mailto:<?php echo e($contact->email); ?>" class="hover:underline"><?php echo e($contact->email); ?></a>
                        </dd>
                    </div>
                    <button type="button" data-copy="<?php echo e($contact->email); ?>"
                            class="js-copy-pill shrink-0 self-center text-slate-400 hover:text-cb-navy p-1.5 rounded-lg hover:bg-white transition"
            ...
            <?php if($contact->relationship): ?>
                <div class="flex items-start gap-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3.5 min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <dd class="mt-0.5 text-sm font-semibold text-cb-navy break-words"><?php echo e($contact->relationship); ?></dd>
                    </div>
                </div>
            <?php endif; ?>
            ...
            <div class="flex items-start gap-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3.5 min-w-0">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <dd class="mt-0.5 text-sm font-semibold text-cb-navy"><?php echo e($contact->created_at->format('d/m/Y')); ?></dd>
                </div>
            </div>
            ...
            <?php if($contact->address): ?>
                <div class="sm:col-span-2 flex items-start gap-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3.5 min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <dd class="mt-0.5 text-sm font-semibold text-cb-navy break-words whitespace-pre-line"><?php echo e($contact->address); ?></dd>
                    </div>
                    <a href="https://www.google.com/maps/search/?api=1&query=<?php echo e(urlencode($contact->address)); ?>"
    ...
    <?php if($contact->notes): ?>
        <div class="cb-card p-5 sm:p-6">
            ...
            <p class="text-sm text-slate-700 leading-relaxed whitespace-pre-line"><?php echo e($contact->notes); ?></p>
        </div>
    <?php endif; ?>
</div>
...
<?php echo $__env->make('layouts.client', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/chandlabook/public_html/resources/views/client/contacts/show.blade.php ENDPATH**/ ?>