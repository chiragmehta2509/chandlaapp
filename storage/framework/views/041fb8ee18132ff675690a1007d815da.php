    ...
    <?php
        ...
        try {
            if (\Illuminate\Support\Facades\Schema::hasTable('subscription_packs')) {
                $dbPacksList = \App\Models\SubscriptionPack::orderBy('min_level', 'asc')->get();
            }
        } catch (\Throwable $e) {}

        if ($dbPacksList && count($dbPacksList) > 0) {
            $plansData = [];
            foreach ($dbPacksList as $p) {
                $features = array_map(function($f) {
                    $highlight = (bool) (str_contains($f, 'Templates') || str_contains($f, 'Unlimited') || str_contains($f, 'UPI') || str_contains($f, '2 Events') || str_contains($f, '3 Events') || str_contains($f, '10 Events') || str_contains($f, 'Family Editors') || str_contains($f, 'Custom Branding') || str_contains($f, 'White Label'));
                    return ['text' => $f, 'highlight' => $highlight];
                }, $p->features ?? []);
                ...
                $plansData[] = [
                    ...
                    'tier'           => $p->min_level,
                    'name'           => $p->name,
                    'price'          => (float) $p->amount_inr,
                    'formattedPrice' => $p->amount_inr == 0 ? '₹0' : ($p->slug === 'enterprise' ? 'Custom' : '₹' . number_format($p->amount_inr, 0)),
                    'period'         => $p->amount_inr == 0 ? 'Free Plan' : ($p->slug === 'enterprise' ? 'Contact Sales' : 'One-Time Payment'),
                    'description'    => $p->description ?? '',
                    'badge'          => $p->badge ?? '',
                    'isPopular'      => (bool) $p->is_popular,
                    'features'       => $features,
                    'limits'         => $p->limits ?? ['events' => 1, 'entries' => 999999, 'qrCollection' => true, 'editors' => 0]
                ];
            }
        } else {
            $buildFeatures = function($key, array $keywords) {
                return array_values(array_map(function($f) use ($keywords) {
                    $highlight = false;
            ...
            $plansData = [
              [
                'id' => 'starter',
                'tier' => 0,
                'name' => config('packs.level_names.0', 'Starter Plan'),
                'price' => 0,
                'formattedPrice' => '₹0',
                'period' => 'Free Plan',
                ...
                'limits' => ['events' => 1, 'entries' => 50, 'qrCollection' => false, 'editors' => 0]
              ],
              [
                'id' => 'celebration',
                'tier' => 1,
                'name' => config('packs.celebration.label', 'Celebration Pack'),
                'price' => (float) config('packs.celebration.amount_inr', 300),
                'formattedPrice' => '₹' . number_format((float) config('packs.celebration.amount_inr', 300), 0),
                'period' => 'One-Time Payment',
                'description' => config('packs.celebration.description', 'Enhance your celebration with printable invitations and graphic studio assets.'),
                'badge' => config('packs.celebration.badge', 'Best Value'),
                'isPopular' => (bool) config('packs.celebration.is_popular', false),
                'features' => $buildFeatures('celebration', ['Templates', 'Creator']),
                'limits' => ['events' => 1, 'entries' => 50, 'qrCollection' => false, 'editors' => 0]
              ],
              [
                'id' => 'guest_contribution',
                'tier' => 2,
                'name' => config('packs.guest_pay_single.label', 'Guest Contribution'),
                'price' => (float) config('packs.guest_pay_single.amount_inr', 400),
                'formattedPrice' => '₹' . number_format((float) config('packs.guest_pay_single.amount_inr', 400), 0),
                'period' => 'One-Time Payment',
                'description' => config('packs.guest_pay_single.description', 'Direct payment collections and unlimited ledger entries for your single event.'),
                'badge' => config('packs.guest_pay_single.badge', 'Recommended'),
                'isPopular' => (bool) config('packs.guest_pay_single.is_popular', false),
                'features' => $buildFeatures('guest_pay_single', ['UPI', 'Unlimited']),
                'limits' => ['events' => 1, 'entries' => 999999, 'qrCollection' => true, 'editors' => 0]
              ],
              [
                'id' => 'host_plus',
                'tier' => 3,
                'name' => config('packs.ledger_duo.label', 'Host Plus Plan'),
                'price' => (float) config('packs.ledger_duo.amount_inr', 500),
                'formattedPrice' => '₹' . number_format((float) config('packs.ledger_duo.amount_inr', 500), 0),
                'period' => 'One-Time Payment',
                'description' => config('packs.ledger_duo.description', 'Manage multiple events with unlimited ledger logs and hosting tools.'),
                'badge' => config('packs.ledger_duo.badge', 'Great Value'),
                'isPopular' => (bool) config('packs.ledger_duo.is_popular', false),
                'features' => $buildFeatures('ledger_duo', ['2 Events', 'Unlimited']),
                'limits' => ['events' => 2, 'entries' => 999999, 'qrCollection' => true, 'editors' => 0]
              ],
              [
                'id' => 'family',
                'tier' => 4,
                'name' => config('packs.family.label', 'Family Plan'),
                'price' => (float) config('packs.family.amount_inr', 600),
                'formattedPrice' => '₹' . number_format((float) config('packs.family.amount_inr', 600), 0),
                'period' => 'One-Time Payment',
                'description' => config('packs.family.description', 'Coordinate family functions together with multi-editor read/write accounts.'),
                'badge' => config('packs.family.badge', 'Family Pick'),
                'isPopular' => (bool) config('packs.family.is_popular', false),
                'features' => $buildFeatures('family', ['Family Editors']),
                'limits' => ['events' => 2, 'entries' => 999999, 'qrCollection' => true, 'editors' => 3]
              ],
              [
                'id' => 'premium_host',
                'tier' => 5,
                'name' => config('packs.premium_bundle.label', 'Premium Host'),
                'price' => (float) config('packs.premium_bundle.amount_inr', 700),
                'formattedPrice' => '₹' . number_format((float) config('packs.premium_bundle.amount_inr', 700), 0),
                'period' => 'One-Time Payment',
                'description' => config('packs.premium_bundle.description', 'Our flagship plan. Elevates everything with premium custom templates and reports.'),
                'badge' => config('packs.premium_bundle.badge', 'Most Popular'),
                'isPopular' => (bool) config('packs.premium_bundle.is_popular', true),
                'features' => $buildFeatures('premium_bundle', ['3 Events', 'Premium', 'Export']),
                'limits' => ['events' => 3, 'entries' => 999999, 'qrCollection' => true, 'editors' => 3]
              ],
              [
                'id' => 'professional',
                'tier' => 6,
                'name' => config('packs.professional.label', 'Professional'),
                'price' => (float) config('packs.professional.amount_inr', 999),
                'formattedPrice' => '₹' . number_format((float) config('packs.professional.amount_inr', 999), 0),
                'period' => 'One-Time Payment',
                'description' => config('packs.professional.description', 'For power users and professional coordinators running multiple large events.'),
                'badge' => config('packs.professional.badge', 'Professional'),
                'isPopular' => (bool) config('packs.professional.is_popular', false),
                'features' => $buildFeatures('professional', ['10 Events', 'Unlimited Family', 'Custom Branding']),
                'limits' => ['events' => 10, 'entries' => 999999, 'qrCollection' => true, 'editors' => 999]
              ],
              [
                'id' => 'enterprise',
                'tier' => 7,
                'name' => config('packs.enterprise.label', 'Enterprise'),
                'price' => (float) config('packs.enterprise.amount_inr', 9999),
                'formattedPrice' => 'Custom',
                'period' => 'Contact Sales',
                'description' => config('packs.enterprise.description', 'Bespoke integration, white labeling, and dedicated hosting for large organizations.'),
                'badge' => config('packs.enterprise.badge', 'Enterprise'),
                'isPopular' => (bool) config('packs.enterprise.is_popular', false),
                'features' => $buildFeatures('enterprise', ['Unlimited', 'White Label']),
                'limits' => ['events' => 9999, 'entries' => 999999, 'qrCollection' => true, 'editors' => 999]
              ]
            ];
        }
    ?>
...
<?php /**PATH /home/chandlabook/public_html/resources/views/partials/pricing-section.blade.php ENDPATH**/ ?>